...
class CBOWDataset(Dataset):
    def __init__(self, pairs, word_to_idx, pair_to_tensor):
        self.pairs = pairs
        self.word_to_idx = word_to_idx
        self.pair_to_tensor = pair_to_tensor

    def __len__(self):
        return len(self.pairs)

    def __getitem__(self, idx):
        return self.pair_to_tensor(self.pairs[idx], self.word_to_idx)
...
class CBOW(nn.Module):
    ...
    def forward(self, inputs):
        embeds = self.embeddings(inputs)  # Look up embeddings for context words
        embeds = torch.mean(embeds, dim=1)  # Average the embeddings
        out = self.linear(embeds)  # Pass through a linear layer
...
class CBOWTrainer:
    ...
    def train(self, train_loader: DataLoader, vocab_size: int, loss_function = nn.NLLLoss()):
        ...
        for epoch in range(self.num_epochs):
            ...
            # test_words = ["dog", "computer", "money"]
            # for test_word in test_words:
            #     similar_words = self.find_similar_words(model, test_word, n=10)
            #     if similar_words:
            #         logging.info(
            #             f"\nTop 10 similar words to '{test_word}' after epoch {epoch + 1}:"
            #         )
